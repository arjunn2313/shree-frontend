...
useEffect(() => {
  ...
  return () => clearInterval(interval);
    ...
    setEntry(prevEntry => {
      const newEntry = prevEntry.slice(0, index) + value + prevEntry.slice(index + 1);
      return newEntry;
    });
    };
...
if(user){
  ...
  const handleVerify = async () => {
      ...
      formData.append('avatar',user.avatar)
      formData.append('otp', entry);
      // const res = await axios.post("http://localhost:6060/user/verify",formData,{
      //   headers: {
    ...
    return (
      <>
        ...
        <div className="container border border-success rounded-4 py-5 my-5">
          <h3 className="text-center">Please enter the one-time password <br/> to verify</h3>
          <h6 className="text-center text-secondary fs-5 py-4">A code has been sent to *******{last3Digits}</h6>
         ...
         {isResendDisabled ? <p className="text-center text-secondary">Resend One-Time Password in
          {timer > 0 && (
            <span style={{color:'green'}}> in {Math.floor(timer / 60)}:{(timer % 60).toString().padStart(2, '0')} minutes</span>
          )}
          
          </p> :
          <p className="text-center text-secondary" onClick={handleResend}>Resend One-Time Password</p>
          }
        </div>
  
        <div className="my-5 d-flex justify-content-center align-items-center">
          {setCurrentStep ?  <button className="otp-btn fs-5" onClick={handleStep}>Verify</button> :
           <button className="otp-btn fs-5" onClick={handleVerify}>Verify</button>
          }
